...
If you deploy BitFabric with Cloudflare Pages Functions + D1, you can enable email verification.

### `POST /api/send-verification`

Creates (or replaces) a verification token for the email and sends a verification message.
...
```json
{ "sent": true, "expiresAt": 1730000000000 }
```

**Notes:**
- Requires `DB` binding.
- Email sending uses MailChannels by default (set `MAIL_FROM`).
- For local/dev you can set `EMAIL_MODE=log` to print the verify URL in logs.

### `GET /api/verify-email?email=...&token=...`

Verifies the email using a token from the verification email.

**Success:**

```json
{ "verified": true, "alreadyVerified": false, "verifiedAt": 1730000000000 }
```

### `POST /api/verify-email`

Same as the GET endpoint, but uses a JSON body.

**Body:**

```json
{ "email": "you@domain.com", "token": "<hex token>" }
```
